...
const IconProfile: ComponentType<IconProps> = ({ className = '' }) => (
  <svg
  className={className}
  viewBox="0 0 24 24"
  fill="none"
...
const NavBar = () => {
  return (
    <nav className="h-full w-50 rounded-3xl bg-[#0f0f10] px-5 py-6 text-white shadow-[0_25px_80px_rgba(0,0,0,0.55)]">
        ...
        {navItems.map(({ label, icon: Icon, to }) => (
          <li key={label}>
            <NavLink
              to={to}
              className={({ isActive }) =>
                `flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-base font-medium transition ${
                  isActive
